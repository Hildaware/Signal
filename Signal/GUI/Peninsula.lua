...
function core:Create()
    ...
    local contentContainer = CreateFrame('Frame', 'SignalCore', UIParent)
...
function core:Dissolve()
    ...
    events:SendMessage('SIGNAL_CORE_END')
...
function core:Precipitate()
    ...
    events:SendMessage('SIGNAL_CORE_START')
...
function events:SIGNAL_ADD_CORE_ITEM(_, widget)
    core:Precipitate()
    core.data:AddChild(widget)
end

---@param widget BasePeninsula
function events:SIGNAL_UPDATE_CORE_ITEM(_, widget)
    core:UpdateHeight(widget)
end

---@param widget BasePeninsula
function events:SIGNAL_ITEM_TIMER_END(_, widget)
